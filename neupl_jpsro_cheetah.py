...
class ActorCritic(nn.Module):
    ...
    def mu_logstd_pop_sg(self, obs, prev_cop_act, h_prev, cond_pop):
        ...
        def _update_epoch(update_state, unused):
            def _update_minibatch(train_state, batch_info):
                ...
                def _loss_fn(params, traj_batch, gae, targets, rng_mb):
                    ...
                    pop_method = (ActorCritic.mu_logstd_pop_sg
                                  if cfg.decouple_encoder
                                  else ActorCritic.mu_logstd_pop)
...
def run_neupl_jpsro(cfg: Config):
    print("=" * 70)
    print("  NeuPL-JPSRO on 2-agent MABrax")
    print("=" * 70)
    ...
    if cfg.wandb:
        ...
        wandb.init(
            ...
            config=asdict(cfg),
            mode=cfg.wandb_mode,
            name=(f"neupl-jpsro-seed{cfg.seed}"
                  if cfg.env_name == "halfcheetah_2x3"
                  else f"neupl-jpsro-{cfg.env_name.replace('|', '-')}-seed{cfg.seed}"),
        )
    ...
    agents = env.agents
    assert len(agents) == 2, (
        f"neupl_jpsro_cheetah assumes 2 players; env '{cfg.env_name}' has "
        f"{len(agents)} agents.")
    ...
    dummy_psi_in = jnp.zeros((2 * cfg.embed_dim,))
    ...
    def _eval_joint(rng, params_0, params_1, embed_0, embed_1):
        return eval_joint_strategy(
            ...
            embed_0, embed_1, cfg.ep_eval_episodes, cfg.ep_eval_steps,
    ...
    for t in range(1, cfg.jpsro_iters + 1):
        ...
        for p in range(2):
            ...
            psi_params[p] = br_info["best_params"]["psi"]
...
def _plot_results(cfg, payoff_history, returns_history):
    try:
        ...
        save_path = os.path.join(cfg.save_dir, f"neupl_jpsro_{env_tag}.png")
...
def main():
    ...
    parser.add_argument("--gpu-profile", type=str, default="local",
                        ...
                        help="Preset bundle for {rollout, PPO, net-size}. "
                             "'local' = 24 GB A5000 class; "
                             "'cluster' = ~48 GB A100/L40S class.")

    parser.add_argument("--jpsro-iters", type=int, default=6)

    parser.add_argument("--br-steps", type=int, default=None,
                        help="BR training PPO updates per JPSRO iter. "
                             "Default: gpu-profile-dependent.")
    parser.add_argument("--num-envs", type=int, default=256,
                        help="Parallel JaxMARL envs (vmap on-GPU).")
    parser.add_argument("--num-steps", type=int, default=1000,
                        help="Rollout length (one full MABrax episode).")
    ...
    parser.add_argument("--br-head-layers", type=int, default=None,
                        help="# hidden Dense layers in GaussianHead (BR). "
                             "Default: 2 (both profiles).")
    parser.add_argument("--pop-head-layers", type=int, default=None,
                        help="# hidden Dense layers in GaussianHead (Pop). "
                             "Default: 2 (both profiles).")

    parser.add_argument("--no-decouple-encoder", action="store_true",
                        help="Disable stop_gradient on the pop-head path of "
                             "the distill and regularise losses.  Default "
                             "leaves decoupling ON (cfg.decouple_encoder=True).")
    ...
    parser.add_argument("--br-steps-scale", type=int, default=None,
                        help="Linear scaling of br_steps with JPSRO iter. "
                             "Default: gpu-profile-dependent.")
    ...
    parser.add_argument(
        ...
        choices=["halfcheetah_2x3", "halfcheetah_6x1", "humanoid_9|8",
                 "walker2d_2x3"],
        help="2-agent MABrax env (N>2 agents rejected at runtime).")
    parser.add_argument(
        "--env-homogenisation", type=str, default="", choices=["", "max", "concat"],
        help="MABrax padding; empty = none; 'max' auto for humanoid_9|8.")

    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--save-dir", type=str, default="results_neupl_cheetah")
    parser.add_argument("--wandb", action="store_true")
    parser.add_argument("--wandb-project", type=str, default="neupl-jpsro-halfcheetah")
