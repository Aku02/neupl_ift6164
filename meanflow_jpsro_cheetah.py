...
class Config:
    ...
    br_eval_det_num_envs: int = 64
    br_eval_det_num_steps: int = 128

    use_payoff_estimator: bool = False
    payoff_estimator_hidden: int = 128
    payoff_estimator_coef: float = 1.0
    payoff_estimator_log_mc: bool = True
    ...
    wandb: bool = False
    wandb_project: str = "neupl-jpsro-halfcheetah"
    wandb_entity: str = ""
...
class FlowActorCritic(nn.Module):
    ...
    def generator_pop_sg(self, obs, prev_cop_act, h_prev, cond_pop, z, t):
...
class PayoffEstimator(nn.Module):

    hidden_dim: int = 128

    @nn.compact
    def __call__(self, nu_concat):
        x = nn.Dense(self.hidden_dim,
                     kernel_init=orthogonal(np.sqrt(2)),
                     bias_init=constant(0.0))(nu_concat)
        x = nn.tanh(x)
        x = nn.Dense(self.hidden_dim,
                     kernel_init=orthogonal(np.sqrt(2)),
                     bias_init=constant(0.0))(x)
...
def train_br(
    ...
    g_embed_cce_const = sigma_cce @ cop_embeds

    psi_estimator = PayoffEstimator(hidden_dim=cfg.payoff_estimator_hidden)
    ...
    def _update_step(runner_state, unused):
        ...
        def _update_epoch(update_state, unused):
            def _update_minibatch(carry, batch_info):
                ...
                def _loss_fn(params, traj_batch, gae, targets, old_p, rng_mb):
                    ...
                    lr_diff = cur_ll - old_ll
                    if cfg.log_ratio_clamp_ste:
                        log_ratio = clamp_ste(
                            lr_diff,
                            lo=-cfg.log_ratio_clip, hi=cfg.log_ratio_clip)
                    else:
                        log_ratio = jnp.clip(
                            lr_diff, -cfg.log_ratio_clip, cfg.log_ratio_clip)
                    ...
                    g_embed_cce_b = jnp.broadcast_to(
                        g_embed_cce_const[None, :], (B, d_emb))
                    pop_method = (FlowActorCritic.generator_pop_sg
                                  if cfg.decouple_encoder
                                  else FlowActorCritic.generator_pop)

                    target_br = gen_call(
                        network.apply, net_params,
                        traj_batch.obs, traj_batch.prev_cop_act,
                        traj_batch.h_prev, g_embed_cce_b,
                        z_d, t_one_d,
                        method=FlowActorCritic.generator_br) * cfg.policy_output_scale
                    if cfg.vel_clip > 0:
                        target_br = jnp.clip(
                            target_br, -cfg.vel_clip, cfg.vel_clip)
                    ...
                    distill_mean = gen_call(
                        ...
                        z_d, t_one_d,
                        method=pop_method) * cfg.policy_output_scale
                    if cfg.vel_clip > 0:
                        distill_mean = jnp.clip(
                            distill_mean, -cfg.vel_clip, cfg.vel_clip)

                    distill_loss = jnp.mean(
                        (distill_mean - target_br) ** 2)
                    ...
                    psi_input = jnp.concatenate(
                        [own_embed_b, traj_batch.cop_embed], axis=-1)
                    psi_pred = psi_estimator.apply(params["psi"], psi_input)
                    psi_loss = 0.5 * jnp.mean((psi_pred - targets) ** 2)
                    ...
                    total = (
                        ...
                        + cfg.payoff_estimator_coef * psi_loss
                    )
        ...
        kl_mean = jnp.mean(epoch_losses[7])
...
def eval_br_deterministic(
    rng, env, network_p, params_p, cop_network, cop_params,
    cop_embeds, sigma_cce, player_idx,
    num_envs, num_steps, cfg: Config,
    action_dim_p: int, action_dim_c: int,
):
    ...
    g_single = sigma_cce @ cop_embeds

    def _step(carry, unused):
        (env_state, obs, h_p, h_c, pa_p, pa_c, rng) = carry
        rng, rng_p, rng_c, rng_step, rng_idx = jax.random.split(rng, 5)

        p_obs = obs[player_agent]
        c_obs = obs[cop_agent]
        g_batch = jnp.broadcast_to(g_single, (num_envs, g_single.shape[-1]))

        raw_p, _, _ = gen_sample(
            network_p.apply, params_p, p_obs, pa_p, h_p, g_batch, rng_p,
            action_dim_p, cfg.policy_output_scale,
            0.0,
            cfg.vel_clip,
            method=FlowActorCritic.generator_br,
        )
        p_action = jnp.tanh(raw_p)

        u_gumbel = jax.random.uniform(rng_idx, (num_envs, num_cop_strats),
                                      minval=1e-20, maxval=1.0)
        gumbel = -jnp.log(-jnp.log(u_gumbel))
        logits = jnp.log(jnp.clip(sigma_cce, 1e-20, 1.0))
        strat_idx = jnp.argmax(logits[None, :] + gumbel, axis=-1)
        c_embed = cop_embeds[strat_idx]
        raw_c, _, _ = gen_sample(
            cop_network.apply, cop_params, c_obs, pa_c, h_c, c_embed, rng_c,
            action_dim_c, cfg.policy_output_scale,
            0.0, cfg.vel_clip,
            method=FlowActorCritic.generator_pop,
        )
        c_action = jnp.tanh(raw_c)

        h_p_new = network_p.apply(
            params_p, p_obs, pa_p, h_p, method=FlowActorCritic.encode)
        h_c_new = cop_network.apply(
            cop_params, c_obs, pa_c, h_c, method=FlowActorCritic.encode)
        ...
        d_p = dones[player_agent][:, None].astype(jnp.float32)
        d_c = dones[cop_agent][:, None].astype(jnp.float32)
        h_p_next = h_p_new * (1.0 - d_p)
        h_c_next = h_c_new * (1.0 - d_c)
        pa_p_next = c_action * (1.0 - d_p)
        pa_c_next = p_action * (1.0 - d_c)

        return ((next_state, next_obs, h_p_next, h_c_next,
                 pa_p_next, pa_c_next, rng),
                rewards[player_agent])

    rng, rng_reset = jax.random.split(rng)
    reset_rngs = jax.random.split(rng_reset, num_envs)
    obs, env_state = jax.vmap(env.reset)(reset_rngs)

    h_p0 = jnp.zeros((num_envs, mem_dim))
    h_c0 = jnp.zeros((num_envs, mem_dim))
    pa_p0 = jnp.zeros((num_envs, action_dim_c))
    pa_c0 = jnp.zeros((num_envs, action_dim_p))

    _, rewards = jax.lax.scan(
        _step,
        (env_state, obs, h_p0, h_c0, pa_p0, pa_c0, rng),
        None, num_steps)
...
def run_meanflow_jpsro(cfg: Config):
    print("=" * 70)
    print(f"  MeanFlow-NFT-JPSRO (MeanFlow + PPO-ratio + NFT) on 2-agent MABrax")
    print("=" * 70)
    ...
    if cfg.wandb:
        ...
        wandb.init(
            ...
            config=asdict(cfg),
            mode=cfg.wandb_mode,
            name=(f"meanflow-jpsro-seed{cfg.seed}"
                  if cfg.env_name == "halfcheetah_2x3"
                  else f"meanflow-jpsro-{cfg.env_name.replace('|', '-')}-seed{cfg.seed}"),
        )
    ...
    agents = env.agents
    assert len(agents) == 2, (
        f"meanflow_jpsro_cheetah currently assumes a 2-player game; "
        f"env '{cfg.env_name}' has {len(agents)} agents ({agents}). "
        f"ant_4x2 (4p) and halfcheetah_6x1 (6p) / hopper_3x1 (3p) will "
        f"need an N-player payoff tensor + CCE solver refactor.")

    obs_dims = {a: env.observation_space(a).shape[0] for a in agents}
    act_dims = {a: env.action_space(a).shape[0] for a in agents}
    print(f"  Env: {cfg.env_name} (kwargs={env_kwargs or 'none'})")
    ...
    print(f"  Log-ratio clamp: ste={cfg.log_ratio_clamp_ste}, "
          f"bound=+/-{cfg.log_ratio_clip}")
    ...
    psi_net = PayoffEstimator(hidden_dim=cfg.payoff_estimator_hidden)
    rng, *psi_rngs = jax.random.split(rng, 3)
    dummy_psi_in = jnp.zeros((1, 2 * cfg.embed_dim))
    psi_params = [psi_net.init(psi_rngs[p], dummy_psi_in) for p in range(2)]
    ...
    for t in range(1, cfg.jpsro_iters + 1):
        ...
        for p in range(2):
            ...
            br_eval_det = float(eval_br_deterministic(
                rng=rng_det,
                env=env,
                network_p=networks[p],
                params_p=best_params["net"],
                cop_network=networks[cop],
                cop_params=ref_net_params[cop],
                cop_embeds=cop_embeds_arr,
                sigma_cce=sigma_cop_jnp,
                player_idx=p,
                num_envs=cfg.br_eval_det_num_envs,
                num_steps=cfg.br_eval_det_num_steps,
                cfg=cfg,
                action_dim_p=action_dims[p],
                action_dim_c=action_dims[cop],
            ))
            ...
            print(f"    BR training: {elapsed:.1f}s, "
                  ...
                  f"det-eval (σ_sde=0): {br_eval_det:.2f}")
            ...
            if cfg.wandb:
                ...
                wandb.log({
                    ...
                    f"br/p{p}_return_eval": br_eval_det,
                }, step=global_br_step)

            net_params[p] = best_params["net"]
            embeds[p][-1] = best_params["embed"]

            psi_params[p] = best_params["psi"]
        ...
        embeds0_stack = jnp.stack(embeds[0])
        embeds1_stack = jnp.stack(embeds[1])
        pairs = jnp.concatenate([
            jnp.broadcast_to(embeds0_stack[:, None, :],
                             (n0, n1, cfg.embed_dim)),
            jnp.broadcast_to(embeds1_stack[None, :, :],
                             (n0, n1, cfg.embed_dim)),
        ], axis=-1).reshape(n0 * n1, 2 * cfg.embed_dim)
        psi_0 = np.array(psi_net.apply(psi_params[0], pairs)).reshape(n0, n1)
        psi_1 = np.array(psi_net.apply(psi_params[1], pairs)).reshape(n0, n1)
        psi_payoff = 0.5 * (psi_0 + psi_1)

        mc_payoff = None
        if cfg.payoff_estimator_log_mc or not cfg.use_payoff_estimator:
            mc_payoff = np.zeros((n0, n1))
            for i in range(n0):
                for j in range(n1):
                    rng, rng_eval = jax.random.split(rng)
                    p0_params = param_snapshots[0][min(i, len(param_snapshots[0]) - 1)]
                    p1_params = param_snapshots[1][min(j, len(param_snapshots[1]) - 1)]
                    mc_payoff[i, j] = float(_eval_joint(
                        rng_eval, p0_params, p1_params,
                        embeds[0][i], embeds[1][j]))

        if cfg.use_payoff_estimator:
            payoff_matrix = psi_payoff.copy()
            print(f"  [ψ_w] Payoff matrix:\n{np.round(psi_payoff, 1)}")
            if mc_payoff is not None:
                err = psi_payoff - mc_payoff
                mae = float(np.mean(np.abs(err)))
                rmse = float(np.sqrt(np.mean(err ** 2)))
                print(f"  [ψ_w vs MC] MAE={mae:.2f}  RMSE={rmse:.2f}  "
                      f"(MC range [{mc_payoff.min():.1f}, {mc_payoff.max():.1f}])")
        else:
            assert mc_payoff is not None
            payoff_matrix = mc_payoff.copy()
            print(f"  [MC]  Payoff matrix:\n{np.round(mc_payoff, 1)}")
        ...
        if cfg.wandb:
            ...
            wandb_payload = {
                ...
                "jpsro/using_psi_w": int(cfg.use_payoff_estimator),
            }
            if mc_payoff is not None:
                err = psi_payoff - mc_payoff
                wandb_payload.update({
                    "payoff/psi_vs_mc_mae": float(np.mean(np.abs(err))),
                    "payoff/psi_vs_mc_rmse": float(np.sqrt(np.mean(err ** 2))),
                    "payoff/mc_min": float(mc_payoff.min()),
                    "payoff/mc_max": float(mc_payoff.max()),
                    "payoff/psi_min": float(psi_payoff.min()),
                    "payoff/psi_max": float(psi_payoff.max()),
                })
            wandb.log(wandb_payload, step=global_br_step)
...
def _plot_results(cfg, payoff_history, returns_history):
    try:
        ...
        plt.tight_layout()
        env_tag = cfg.env_name.replace('|', '-')
        save_path = os.path.join(cfg.save_dir, f"meanflow_jpsro_{env_tag}.png")
...
def main():
    ...
    parser.add_argument("--br-head-layers", type=int, default=None,
                        help="# hidden Dense layers in BR GeneratorHead. "
                             "Default: 2 (local) / 3 (cluster).")
    parser.add_argument("--pop-head-layers", type=int, default=None,
                        help="# hidden Dense layers in Pop GeneratorHead. "
                             "Default: 2 (local) / 3 (cluster).")
    ...
    parser.add_argument("--no-log-ratio-clamp-ste", action="store_true",
                        help="disable straight-through clamp on log-ratio "
                             "(revert to jnp.clip which zeros gradients at "
                             "the log-ratio boundary -- NOT recommended)")
    ...
    parser.add_argument("--desired-kl", type=float, default=0.02,
                        help="target approx_kl for adaptive LR (MeanFlow "
                             "default 0.02; FPPO uses 0.01)")
    ...
    parser.add_argument("--br-eval-det-num-envs", type=int, default=64)
    parser.add_argument("--br-eval-det-num-steps", type=int, default=128)

    parser.add_argument("--payoff-estimator", action="store_true",
                        help="Enable learned ψ_w payoff estimator. "
                             "Default: off (use MC for payoff tensor).")
    parser.add_argument("--no-payoff-estimator", action="store_true",
                        help="(Legacy / no-op -- ψ_w is off by default now.)")
    parser.add_argument("--payoff-estimator-hidden", type=int, default=128)
    parser.add_argument("--payoff-estimator-coef", type=float, default=1.0,
                        help="Weight of ψ_w MSE loss in combined BR loss.")
    parser.add_argument("--no-payoff-estimator-log-mc", action="store_true",
                        help="Skip MC rollouts when using ψ_w "
                             "(default: also run MC for diagnostic MAE/RMSE).")

    parser.add_argument(
        "--env-name", type=str, default="halfcheetah_2x3",
        choices=["halfcheetah_2x3", "halfcheetah_6x1", "humanoid_9|8",
                 "walker2d_2x3"],
        help="2-agent MABrax env. halfcheetah_6x1 is 6-agent and will be "
             "rejected by the 2-player assertion (listed for future N-player "
             "refactor). N-player envs (ant_4x2, hopper_3x1) are not yet "
             "supported -- see follow-up work.")
    parser.add_argument(
        "--env-homogenisation", type=str, default="", choices=["", "max", "concat"],
        help="MABraxEnv padding method. Empty = none. 'max' auto-enabled "
             "for humanoid_9|8 (asymmetric action dims 9,8) unless overridden.")

    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--save-dir", type=str, default="results_meanflow_cheetah")
    parser.add_argument("--wandb", action="store_true")
    parser.add_argument("--wandb-project", type=str,
                        default="neupl-jpsro-halfcheetah")
    ...
    use_psi = bool(args.payoff_estimator) and (not args.no_payoff_estimator)
    ...
    cfg = Config(
        ...
        log_ratio_clamp_ste=(not args.no_log_ratio_clamp_ste),
        ...
        br_eval_det_num_envs=args.br_eval_det_num_envs,
        br_eval_det_num_steps=args.br_eval_det_num_steps,
        use_payoff_estimator=use_psi,
        payoff_estimator_hidden=args.payoff_estimator_hidden,
        payoff_estimator_coef=args.payoff_estimator_coef,
        payoff_estimator_log_mc=(not args.no_payoff_estimator_log_mc),
