    ...
    with torch.no_grad():
        ...
        for x in x_grid:
            ...
            for j, weight in enumerate(col_mix):
                if weight <= 1e-10:
                    continue
                if isinstance(population, MixedPopulation):
                    value += float(
                        weight
                        * (cartesian_payoff(x_t, population.y_grid) @ population.col_policy(j)).item()
                    )
                else:
                    value += float(
                        weight * cartesian_payoff(x_t, population.col_action(j)).item()
                    )
        ...
        for y in y_grid:
            ...
            for i, weight in enumerate(row_mix):
                ...
                if isinstance(population, MixedPopulation):
                    value += float(
                        weight
                        * (population.row_policy(i) @ cartesian_payoff(population.x_grid, y_t)).item()
                    )
                else:
                    value += float(
                        weight * cartesian_payoff(population.row_action(i), y_t).item()
                    )
...
class PurePopulation(BasePopulation):
    def __init__(self, num_slots: int, game_cfg: GameConfig):
        super().__init__(num_slots, game_cfg)
        self.row_raw = nn.ParameterList(
            [nn.Parameter(0.25 * torch.randn(())) for _ in range(num_slots)]
        )
        self.col_raw = nn.ParameterList(
            [nn.Parameter(0.25 * torch.randn(())) for _ in range(num_slots)]
        )

    def row_action(self, idx: int) -> torch.Tensor:
        return bounded_scalar(self.row_raw[idx], self.game_cfg.xmin, self.game_cfg.xmax)

    def col_action(self, idx: int) -> torch.Tensor:
        return bounded_scalar(self.col_raw[idx], self.game_cfg.ymin, self.game_cfg.ymax)
    ...
    def init_anchor(self, x0: float, y0: float):
        ...
        with torch.no_grad():
            self.row_raw[0].copy_(torch.tensor(inv_sigmoid(x_scaled)))
            self.col_raw[0].copy_(torch.tensor(inv_sigmoid(y_scaled)))

    def expected_row_action(self, idx: int) -> torch.Tensor:
        return self.row_action(idx)

    def expected_col_action(self, idx: int) -> torch.Tensor:
        return self.col_action(idx)

    def evaluate_payoff_matrix(self) -> np.ndarray:
        payoff = np.zeros((self.num_slots, self.num_slots), dtype=np.float32)
        with torch.no_grad():
            for i in range(self.num_slots):
                x_i = self.row_action(i)
                for j in range(self.num_slots):
                    payoff[i, j] = float(cartesian_payoff(x_i, self.col_action(j)).item())
        return payoff

    def row_loss(self, sigma: torch.Tensor, slot_i: int) -> torch.Tensor:
        x_i = self.row_action(slot_i)
        value = torch.tensor(0.0)
        for j, weight in enumerate(sigma[slot_i]):
            w = float(weight.item())
            if w > 1e-8:
                value = value + w * cartesian_payoff(x_i, self.col_action(j).detach())
        return -value

    def col_loss(self, sigma: torch.Tensor, slot_j: int) -> torch.Tensor:
        y_j = self.col_action(slot_j)
        value = torch.tensor(0.0)
        for i, weight in enumerate(sigma[slot_j]):
            w = float(weight.item())
            if w > 1e-8:
                value = value + w * cartesian_payoff(self.row_action(i).detach(), y_j)
        return value

    def summarize_slot(self, idx: int) -> tuple[float, float, float]:
        with torch.no_grad():
            x = float(self.row_action(idx).item())
            y = float(self.col_action(idx).item())
            value = float(cartesian_payoff(torch.tensor(x), torch.tensor(y)).item())
        return x, y, value
    ...
    def meta_density(
        ...
        x_support = np.array([self.summarize_slot(i)[0] for i in range(self.num_slots)])
        y_support = np.array([self.summarize_slot(i)[1] for i in range(self.num_slots)])
        x_grid = np.linspace(self.game_cfg.xmin, self.game_cfg.xmax, 400)
        y_grid = np.linspace(self.game_cfg.ymin, self.game_cfg.ymax, 400)
        x_density = gaussian_mixture_density(
            x_support, row_weights, x_grid, 0.08 * (self.game_cfg.xmax - self.game_cfg.xmin)
        )
        y_density = gaussian_mixture_density(
            y_support, col_weights, y_grid, 0.08 * (self.game_cfg.ymax - self.game_cfg.ymin)
        )
        return x_grid, y_grid, x_density, y_density

class MixedPopulation(BasePopulation):
    def __init__(self, num_slots: int, game_cfg: GameConfig):
        super().__init__(num_slots, game_cfg)
        self.row_logits = nn.ParameterList(
            [nn.Parameter(0.05 * torch.randn(game_cfg.num_bins)) for _ in range(num_slots)]
        )
        self.col_logits = nn.ParameterList(
            [nn.Parameter(0.05 * torch.randn(game_cfg.num_bins)) for _ in range(num_slots)]
        )

    def row_policy(self, idx: int) -> torch.Tensor:
        return torch.softmax(self.row_logits[idx], dim=-1)

    def col_policy(self, idx: int) -> torch.Tensor:
        return torch.softmax(self.col_logits[idx], dim=-1)

    def init_anchor(self, x0: float, y0: float):
        with torch.no_grad():
            self.row_logits[0].copy_(-((self.x_grid - x0) / 0.12) ** 2)
            self.col_logits[0].copy_(-((self.y_grid - y0) / 0.12) ** 2)

    def expected_row_action(self, idx: int) -> torch.Tensor:
        return (self.row_policy(idx) * self.x_grid).sum()

    def expected_col_action(self, idx: int) -> torch.Tensor:
        return (self.col_policy(idx) * self.y_grid).sum()

    def evaluate_payoff_matrix(self) -> np.ndarray:
        payoff = np.zeros((self.num_slots, self.num_slots), dtype=np.float32)
        with torch.no_grad():
            for i in range(self.num_slots):
                row_pi = self.row_policy(i)
                for j in range(self.num_slots):
                    payoff[i, j] = float((row_pi @ self.payoff_grid @ self.col_policy(j)).item())
        return payoff

    def row_loss(self, sigma: torch.Tensor, slot_i: int) -> torch.Tensor:
        row_pi = self.row_policy(slot_i)
        value = torch.tensor(0.0)
        for j, weight in enumerate(sigma[slot_i]):
            w = float(weight.item())
            if w > 1e-8:
                value = value + w * (row_pi @ self.payoff_grid @ self.col_policy(j).detach())
        return -value

    def col_loss(self, sigma: torch.Tensor, slot_j: int) -> torch.Tensor:
        col_pi = self.col_policy(slot_j)
        value = torch.tensor(0.0)
        for i, weight in enumerate(sigma[slot_j]):
            w = float(weight.item())
            if w > 1e-8:
                value = value + w * (self.row_policy(i).detach() @ self.payoff_grid @ col_pi)
        return value

    def summarize_slot(self, idx: int) -> tuple[float, float, float]:
        with torch.no_grad():
            row_pi = self.row_policy(idx)
            col_pi = self.col_policy(idx)
            x = float((row_pi * self.x_grid).sum().item())
            y = float((col_pi * self.y_grid).sum().item())
            value = float((row_pi @ self.payoff_grid @ col_pi).item())
        return x, y, value
    ...
    def meta_density(
        ...
        row_density = np.zeros(self.game_cfg.num_bins, dtype=np.float64)
        col_density = np.zeros(self.game_cfg.num_bins, dtype=np.float64)
        for idx, weight in enumerate(row_weights):
            row_density += weight * self.row_policy(idx).detach().cpu().numpy()
        for idx, weight in enumerate(col_weights):
            col_density += weight * self.col_policy(idx).detach().cpu().numpy()
        return (
            self.x_grid.cpu().numpy(),
            self.y_grid.cpu().numpy(),
            row_density,
            col_density,
        )
...
def make_population(model: str, num_slots: int, game_cfg: GameConfig) -> BasePopulation:
    if model == "pure":
        return PurePopulation(num_slots, game_cfg)
    if model == "discrete_mixed":
        return MixedPopulation(num_slots, game_cfg)
    raise ValueError(f"Unknown policy model: {model}")

def main():
    parser = argparse.ArgumentParser(description="Adaptive NeuPL for a Cartesian game")
    ...
    parser.add_argument("--save-dir", type=str, default="cartesian_results")
    ...
    parser.add_argument("--policy-model", type=str, default="pure", choices=["pure", "discrete_mixed"])
    parser.add_argument(
        "--graph",
        type=str,
        default="psro_nash",
    ...
    model_tag = f"neupl_{args.policy_model}"
    ...
    print("\n" + "=" * 60)
    print(f"  NeuPL ({graph_label}, N={args.adaptive_n}, model={args.policy_model})")
    print("=" * 60)
    ...
    population = make_population(args.policy_model, args.adaptive_n, game_cfg)
    ...
    print_final_summary(population, f"NeuPL-{graph_label}")

    plot_trajectories(
        row_history,
        col_history,
        game_cfg,
        f"NeuPL {graph_label} ({args.policy_model})",
