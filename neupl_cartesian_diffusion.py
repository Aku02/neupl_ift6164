...
class TrainConfig:
    ...
    diffusion_support_size: int = 7
    diffusion_hidden: int = 64
    diffusion_noise: float = 1.0
    diffusion_aux_coef: float = 0.1
...
class ScalarDenoiser(nn.Module):
...
class DiffusionPopulation(BasePopulation):
    def __init__(self, num_slots: int, game_cfg: GameConfig, cfg: TrainConfig):
        super().__init__(num_slots, game_cfg)
        self.cfg = cfg
        k = cfg.diffusion_support_size

        self.row_clean_raw = nn.ParameterList(
            [nn.Parameter(0.15 * torch.randn(k)) for _ in range(num_slots)]
        )
        self.col_clean_raw = nn.ParameterList(
            [nn.Parameter(0.15 * torch.randn(k)) for _ in range(num_slots)]
        )
        ...
        self.row_denoisers = nn.ModuleList(
            [ScalarDenoiser(cfg.diffusion_hidden) for _ in range(num_slots)]
        )
        self.col_denoisers = nn.ModuleList(
            [ScalarDenoiser(cfg.diffusion_hidden) for _ in range(num_slots)]
        )

        row_noise = cfg.diffusion_noise * torch.randn(num_slots, k)
        col_noise = cfg.diffusion_noise * torch.randn(num_slots, k)
        self.register_buffer("row_noise_anchors", row_noise)
        self.register_buffer("col_noise_anchors", col_noise)

    def _generate_latent(self, idx: int, player: str) -> torch.Tensor:
        if player == "row":
            denoiser = self.row_denoisers[idx]
            x = self.row_noise_anchors[idx].clone()
        else:
            denoiser = self.col_denoisers[idx]
            x = self.col_noise_anchors[idx].clone()

        t_grid = torch.linspace(1.0, 0.0, self.cfg.diffusion_steps + 1, device=x.device)
        x = x.unsqueeze(-1)
        for step_i in range(self.cfg.diffusion_steps):
            t_cur = torch.full((x.shape[0], 1), float(t_grid[step_i].item()), device=x.device)
            dt = t_grid[step_i + 1] - t_grid[step_i]
            x = x + dt * denoiser(x, t_cur)
        return x.squeeze(-1)

    def _slot_support(self, idx: int, player: str) -> tuple[torch.Tensor, torch.Tensor]:
        if player == "row":
            weights = torch.softmax(self.row_logits[idx], dim=-1)
            support = bounded_scalar(self._generate_latent(idx, "row"), self.game_cfg.xmin, self.game_cfg.xmax)
        else:
            weights = torch.softmax(self.col_logits[idx], dim=-1)
            support = bounded_scalar(self._generate_latent(idx, "col"), self.game_cfg.ymin, self.game_cfg.ymax)
        return weights, support

    def _aux_loss(self, idx: int, player: str) -> torch.Tensor:
        if player == "row":
            clean_raw = self.row_clean_raw[idx]
            denoiser = self.row_denoisers[idx]
        else:
            clean_raw = self.col_clean_raw[idx]
            denoiser = self.col_denoisers[idx]

        noise = torch.randn_like(clean_raw) * self.cfg.diffusion_noise
        t = torch.rand(clean_raw.shape[0], 1, device=clean_raw.device).clamp(min=1e-3)
        x0 = clean_raw.unsqueeze(-1)
        x_t = (1.0 - t) * x0 + t * noise.unsqueeze(-1)
        x0_pred = x_t - t * denoiser(x_t, t)
        return ((x0_pred.squeeze(-1) - clean_raw) ** 2).mean()
    ...
    def init_anchor(self, x0: float, y0: float):
        ...
        k = self.cfg.diffusion_support_size
        jitter = torch.linspace(-0.4, 0.4, k)
        with torch.no_grad():
            self.row_clean_raw[0].copy_(row_center + jitter)
            self.col_clean_raw[0].copy_(col_center + jitter)
    ...
    def row_loss(self, sigma: torch.Tensor, slot_i: int) -> torch.Tensor:
        ...
        return -value + self.cfg.diffusion_aux_coef * self._aux_loss(slot_i, "row")
    ...
    def col_loss(self, sigma: torch.Tensor, slot_j: int) -> torch.Tensor:
        ...
        return value + self.cfg.diffusion_aux_coef * self._aux_loss(slot_j, "col")
...
def compute_diversity_metrics(
    ...
    return {
        "n_effective_row": float(n_eff_row),
        "n_effective_col": float(n_eff_col),
...
def main():
    parser = argparse.ArgumentParser(description="NeuPL diffusion variant for a Cartesian game")
    ...
    parser.add_argument("--save-dir", type=str, default="cartesian_diffusion_results")
    ...
    parser.add_argument("--diffusion-support-size", type=int, default=7)
    parser.add_argument("--diffusion-hidden", type=int, default=64)
    parser.add_argument("--diffusion-noise", type=float, default=1.0)
    parser.add_argument("--diffusion-aux-coef", type=float, default=0.1)
    parser.add_argument(
        "--graph",
        type=str,
        default="psro_nash",
    ...
    cfg = TrainConfig(
        ...
        diffusion_support_size=args.diffusion_support_size,
        diffusion_hidden=args.diffusion_hidden,
        diffusion_noise=args.diffusion_noise,
        diffusion_aux_coef=args.diffusion_aux_coef,
    )
    ...
    print("\n" + "=" * 60)
    print(f"  NeuPL ({graph_label}, N={args.adaptive_n}, model=diffusion)")
    print("=" * 60)
    ...
    population = DiffusionPopulation(args.adaptive_n, game_cfg, cfg)
    ...
    print_final_summary(population, f"NeuPL-{graph_label}-diffusion")

    plot_trajectories(
        row_history,
        col_history,
        game_cfg,
        f"NeuPL {graph_label} (diffusion)",
    ...
    print("\n  Diversity metrics:")
    print(f"    effective slots    = row {int(div['n_effective_row'])}, col {int(div['n_effective_col'])}")
